                                      ...
                                      ke_task_id_t const src_id)
{
    ...
    {
        memcpy(&wpts_env.adv_interval, param, 2);
        if (wpts_env.adv_interval >= 0x20 &&  wpts_env.adv_interval <= 0x4000)
        {
            memcpy(wpts_env.dev_name, &param[2], 16);
            // make sure string is ended by 0x00
            wpts_env.dev_name[16] = 0x00;
            memcpy(wpts_env.scan_rsp_data, &param[2 + 16], 16);
            app_adv_start();
