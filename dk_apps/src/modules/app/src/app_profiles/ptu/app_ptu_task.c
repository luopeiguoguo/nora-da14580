...
static bool is_power_adjustment_finished(void)
{
    ...
    for (i = 0; i < PTU_IMPL_MAX_SUPPORTED_DEV; ++i)
    {
        ...
        if (wdev->used &&
                (wdev->pub_data.ptu_dev_state == DEV_PTU_PWDADJ_NEW_REG_WR
                        ||wdev->pub_data.ptu_dev_state == DEV_PTU_PWADJ_NEW_REG_DYN_RD_CFM
                        || wdev->pub_data.ptu_dev_state == DEV_PTU_PWADJ_IN_POWER_TRANSFER_PRU_CTRL_WR
                        || wdev->pub_data.ptu_dev_state == DEV_PTU_PWADJ_IN_POWER_TRANSFERR_DYN_RD_CFM))
        {
...
static wpts_dev * get_dev_in_registration_phase(void)
{
    ...
    for (i = 0; i < PTU_IMPL_MAX_SUPPORTED_DEV; ++i)
    {
        if (app_ptu_env.wpts_devices[i].used &&
                app_ptu_env.wpts_devices[i].pub_data.ptu_dev_state < DEV_PTU_DYN_READ_CHARGING)
        {
...
static void handle_registration_error(wpts_dev *wdev, uint8_t err)
{
    ...
    if (ke_state_get(TASK_APP_PTU) == APP_PTU_LOW_POWER_STATE)
    {
        ...
        wdev->pub_data.ptu_dev_state = DEV_PTU_DISCONNECTING;
...
static float get_pru_real_power_draw(wpts_dev *wdev)
{
    float pru_power = ((float)wdev->pub_data.pru_static_param.p_rect_max * 0.1f); // MAX possible power in Watts

    switch(wdev->shared_mode_val)
    {
...
static bool is_enough_power_for_all_devices(void)
{
    int i;
    ...
    for (i = 0; i < PTU_IMPL_MAX_SUPPORTED_DEV; ++i)
    {
        ...
        if (wdev->used)
        {
            wdev->pub_data.curr_power_draw = get_pru_real_power_draw(wdev);
...
static bool is_pru_category_ok(uint8_t pru_category)
{
    switch (app_ptu_env.ptu_stat_val.ptu_class)
    {
    case WPTC_CLASS_1:
        if (pru_category > WPTS_STATIC_CATEGORY_1)
            return false;
        else
            return true;
    case WPTC_CLASS_2:
        if (pru_category > WPTS_STATIC_CATEGORY_3)
            return false;
        else
            return true;
    case WPTC_CLASS_3:
        if (pru_category > WPTS_STATIC_CATEGORY_4)
            return false;
        else
            return true;
    case WPTC_CLASS_4:
        if (pru_category > WPTS_STATIC_CATEGORY_5)
            return false;
        else
            return true;
    case WPTC_CLASS_5:
        if (pru_category > WPTS_STATIC_CATEGORY_6)
            return false;
        else
            return true;
    case WPTC_CLASS_6:
        return true; // supporting all powwible PRU categories
    default:
        return false;
    }
}
...
static bool limit_all_pru_drain(uint8_t sharing_mode_val)
{
    ...
    for (i = 0; i < PTU_IMPL_MAX_SUPPORTED_DEV; ++i)
    {
                ...
                && is_pru_shared_mode_supported(&app_ptu_env.wpts_devices[i]))
        {
            ...
            if (is_enough_power_for_all_devices())
            {
...
static bool increase_all_pru_drain(uint8_t sharing_mode_val)
{
    ...
    for (i = 0; i < PTU_IMPL_MAX_SUPPORTED_DEV; ++i)
    {
                ...
                && is_pru_shared_mode_supported(wdev))
        {
               ...
               wdev->shared_mode_val = sharing_mode_val;

               if (!is_enough_power_for_all_devices())
               {
                   // no possibility to increase power anymore, reverting last change
                   wdev->shared_mode_val = prev_shared_val;
...
static void adjust_optimal_power_all_wdevs_up(void)
{
    ...
    power_available = increase_all_pru_drain(WPTS_CTRL_ADJ_POW_2_5_W);
    ...
    if (power_available)
        limit_all_pru_drain(WPTS_CTRL_ADJ_POW_MAX);
...
bool app_ptu_task_handle_power_sharing(wpts_dev *added_wdev, bool adjust_low)
{
    ...
    if (power_available)
    {
        if (adjust_low && added_wdev)
        {
            ...
            if (!is_pru_shared_mode_supported(added_wdev)
                    || added_wdev->shared_mode_val == WPTS_CTRL_ADJ_POW_MAX)
            {
                // PRU should wait for available power until every device responds for
                // power adjust
                added_wdev->pub_data.ptu_dev_state = DEV_PTU_PWADJ_PRU_CTRL_WAIT_FOR_POWER_PERM_WR;
            }
        }
        ...
        for (i = 0; i < PTU_IMPL_MAX_SUPPORTED_DEV; ++i)
        {
            ...
            {
                if (wdev->pub_data.ptu_dev_state < DEV_PTU_DYN_READ_CHARGING)
                    wdev->pub_data.ptu_dev_state = DEV_PTU_PWDADJ_NEW_REG_WR;
                else
                    wdev->pub_data.ptu_dev_state = DEV_PTU_PWADJ_IN_POWER_TRANSFER_PRU_CTRL_WR;
    ...
    else if (adjust_low) // not enough power
    {
        ...
        for (i = 0; i < PTU_IMPL_MAX_SUPPORTED_DEV; ++i)
        {
            if (app_ptu_env.wpts_devices[i].used)
            {
                app_ptu_env.wpts_devices[i].shared_mode_val = app_ptu_env.wpts_devices[i].prev_shared_mode_val;
            }
        }
        ...
        if (added_wdev)
        {
            ...
            // and disconnect
            added_wdev->pub_data.ptu_dev_state = DEV_PTU_PRU_CTRL_ERR_REJECT_PRU_CHARGING;
...
static void handle_dev_state_chgs(wpts_dev *wdev, uint8_t err)
{
    switch(wdev->pub_data.ptu_dev_state)
    {
        ...
        break;
        case DEV_PTU_PRUST_RD:
        {
            wdev->dev_data_changed = true;

            if (err == APP_PTU_NO_ERROR)
            {
                wdev->pub_data.ptu_dev_state = DEV_PTU_PTUST_WR;
            }
            else
            {
        ...
        case DEV_PTU_DYN_RD1:
        {
            ...
            if (err == APP_PTU_NO_ERROR || err == APP_PTU_ERR_ALERT_CHARGE_PORT_ACTIVATED)
            {
                ...
                // read operation ended with success, so now we can check if we have
                // enough power to charge additional device with full power or if
                // pru category is okay for us
                if (!is_pru_category_ok(wdev->pub_data.pru_static_param.category))
                {
                    // category for PRU to high (to high draining power demands for us)
                    wdev->pub_data.ptu_dev_state = DEV_PTU_PRU_CTRL_ERR_REJECT_PRU_CHARGING;
                    app_ptu_send_ext_error_ntf(APP_PTU_ERR_PRU_CATEGORY_TO_HIGH, wdev->pub_data.ble_conn_handle);
                }
                else if (!is_enough_power_for_all_devices())
            {
                ...
                else
                {
                    ...
                    wdev->pub_data.ptu_dev_state = DEV_PTU_PRU_CTRL_START_CHARGING;
                    app_ptu_state_set_with_notify(APP_PTU_POWER_TRANSFER_STATE);
                }
            }
            else if (err == APP_PTU_ERR_ALERT_CHARGE_COMPLETE)
            {
                wdev->pub_data.ptu_dev_state = DEV_PTU_PRU_CTRL_STOP_CHARGING;
            }
            else
            {
                handle_registration_error(wdev, err);
        ...
        break;
        case DEV_PTU_PWDADJ_NEW_REG_WR:
        {
            if (err == APP_PTU_NO_ERROR)
            {
                wdev->pub_data.ptu_dev_state = DEV_PTU_PWADJ_NEW_REG_DYN_RD_CFM;
        ...
        break;
        case DEV_PTU_PWADJ_PRU_CTRL_WAIT_FOR_POWER_PERM_WR:
        {
            if (err == APP_PTU_NO_ERROR)
            {
        ...
        break;
        case DEV_PTU_PRU_CTRL_START_CHARGING:
        {
            wdev->dev_data_changed = true;
            if (err == APP_PTU_NO_ERROR)
            {

                wdev->pub_data.ptu_dev_state = DEV_PTU_PRU_ALERT_NOTIF_REGISTER;
        ...
        break;
        case DEV_PTU_PRU_ALERT_NOTIF_REGISTER:
        {
            wdev->dev_data_changed = true;
            if (err == APP_PTU_NO_ERROR)
            {
                //arch_printf("APP_PTU_TASK pru notif wr cfm =>> DEV_PTU_DYN_READ_CHARGING\n");
                wdev->pub_data.ptu_dev_state = DEV_PTU_DYN_READ_CHARGING;
            }
            else
            {
        ...
        break;
        case DEV_PTU_DYN_READ_CHARGING:
        {
        ...
        break;
        case DEV_PTU_PWADJ_IN_POWER_TRANSFER_PRU_CTRL_WR:
        {
            wdev->dev_data_changed = true;
            if (err == APP_PTU_NO_ERROR)
            {
        ...
        break;
        case DEV_PTU_PWADJ_IN_POWER_TRANSFERR_DYN_RD_CFM:
        {
            if (err == APP_PTU_ERR_ALERT_DYN_POWER_ADJUST_OK)
            {
                wdev->dev_data_changed = true;
                ...
                if (is_power_adjustment_finished())
                {
                    wpts_dev *dev_in_reg = get_wpts_dev_by_dev_state(DEV_PTU_PWADJ_WAIT_FOR_OTHER_PRU_POW_ADJUST);
                    if (dev_in_reg)
                    {
                        dev_in_reg->pub_data.ptu_dev_state = DEV_PTU_PRU_CTRL_START_CHARGING;
        ...
        break;
        case DEV_PTU_PRU_CTRL_STOP_CHARGING:
        case DEV_PTU_PRU_CTRL_ERR_REJECT_PRU_CHARGING:
        {
    ...
    if (param->status == CO_ERROR_NO_ERROR)
    {
        ...
        if (wdev)
        {
            ...
            else
            {
                ...
                wdev->pub_data.ptu_dev_state = DEV_PTU_PRUST_RD;
            }
...
{
    if (wdev->pub_data.pru_dynamic_param.pru_alert != 0)
    {
        handle_pru_alert(wdev, false, wdev->pub_data.pru_dynamic_param.pru_alert);
    }
    else
    {
        // NOTE: here put standard validation for PRU handle
        handle_dev_state_chgs(wdev, APP_PTU_NO_ERROR); // NO_ERROR
    }
}
...
static void check_pru_dynamic_changed(wpts_dev *wdev, struct wptc_rd_char_rsp *param)
{
    ...
    if (0 == memcmp(&pru_dynamic_fresh, &wdev->pub_data.pru_dynamic_param, sizeof(struct pru_dynamic_value))
            && wdev->pub_data.ptu_dev_state > DEV_PTU_DYN_RD1) // only print dots in continuus read mode
    {
                                ...
                                ke_task_id_t const src_id)
{
    ...
    for (i = 0; i < PTU_IMPL_MAX_SUPPORTED_DEV; ++i)
    {
        ...
        if (wdev->used)
        {
            ...
            if (wdev->pub_data.ptu_dev_state == DEV_PTU_PWDADJ_NEW_REG_WR
                    || wdev->pub_data.ptu_dev_state == DEV_PTU_PWADJ_NEW_REG_DYN_RD_CFM
                    || wdev->pub_data.ptu_dev_state == DEV_PTU_PWADJ_PRU_CTRL_WAIT_FOR_POWER_PERM_WR
                    || wdev->pub_data.ptu_dev_state == DEV_PTU_PWADJ_WAIT_FOR_OTHER_PRU_POW_ADJUST)
            {
                                ...
                                ke_task_id_t const src_id)
{
    ...
    {
        wpts_dev *wdev = &(app_ptu_env.wpts_devices[i]);
        if (wdev->used && wdev->pub_data.ptu_dev_state == DEV_PTU_PWADJ_IN_POWER_TRANSFERR_DYN_RD_CFM)
        {
            // rejecting charging because PRU could not adjust its power to lower value in 1 second
            wdev->pub_data.ptu_dev_state = DEV_PTU_DYN_READ_CHARGING;
