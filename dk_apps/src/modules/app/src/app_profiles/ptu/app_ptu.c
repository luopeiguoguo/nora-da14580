 ...
 */
bool app_ptu_check_connecting_possible(struct bd_addr const *peer_addr)
{
    int i;
    ...
    // is active
    if (!app_ptu_env.load_detected && !ke_timer_active(APP_PTU_TIMER_MODE_TRANSITION_EMPTY, TASK_APP)
    ...
    {
        app_ptu_gtl_notify_discovery_event(APP_PTU_DISC_ERR_BAD_PTU_STATE);
        return false;
    }
    ...
    // resetting load detercted
    app_ptu_env.load_detected = false;
                            ...
                            struct bd_addr const *peer_addr, uint8_t wpt_adv, uint8_t rssi_flags_adv)
{
    ...
    dev->pub_data.ptu_dev_state = DEV_PTU_IDLE;
    dev->used = true;
...
{
    struct app_ext_ptu_time_set_check_req *msg = KE_MSG_ALLOC(APP_PTU_EXT_VALIDATE_TIME_SET_REQ,
                                                         TASK_GTL, TASK_APP,
...
static void app_ptu_write_pru_control(wpts_dev *wdev, uint8_t enables_val, uint8_t permission_val)
{
    ...
    if (enables_val && wdev->pub_data.time_set_supported_adv && app_ptu_env.time_set_default_val)
    {
...
{
    dev->pub_data.ptu_dev_state = DEV_PTU_DISCONNECTING;
...
static void dev_state_handler(wpts_dev *wdev)
{
    ...
    switch (wdev->pub_data.ptu_dev_state)
    {
        ...
        break;
        case DEV_PTU_PWADJ_WAIT_FOR_OTHER_PRU_POW_ADJUST:
        {
        ...
        {
            // in this state we have to register for nitofications
            if (!wdev->wptc_trans_in_prog)
            {
        ...
        case DEV_PTU_PRU_CTRL_ERR_REJECT_PRU_CHARGING:
        {
            ...
            if (!wdev->wptc_trans_in_prog)
                app_ptu_write_pru_control(wdev, WPTS_CTRL_DISABLE_PRU_OUTPUT, WPTS_CTRL_PERM_DENIED_LIMIT_POW);
        }
        break;
        case DEV_PTU_DISCONNECTING:
        {
            if (!wdev->wptc_trans_in_prog)
                do_disconnect(wdev);
        }
...
static void disconnect_all_wdev_in_fault_state(wpts_dev *wdev)
{
                                            ...
                                            // are disconnected
    {
        ...
        {
            if (app_ptu_env.wpts_devices[i].used
                    && app_ptu_env.wpts_devices[i].pub_data.ptu_dev_state != DEV_PTU_DISCONNECTED
                    && app_ptu_env.wpts_devices[i].pub_data.ptu_dev_state != DEV_PTU_DISCONNECTING)
            {
...
static void handle_wdev_disconnect_with_ptx_variation(wpts_dev *wdev, uint16_t connhdl)
{
...
{
    int i;
	bool dev_removed_ptxin_disconnect = false;
                ...
                disc_ind_param->conhdl, GAP_INVALID_CONIDX, false))
        {
            ...
            else
            {
                if (app_ptu_env.ptxin_variation_detect)
                {
                    ...
                    // A4WP #5.2.2.2
                    dev_removed_ptxin_disconnect = true;
                    handle_wdev_disconnect_with_ptx_variation(&(app_ptu_env.wpts_devices[i]),
                                                               disc_ind_param->conhdl);
            ...
            if (!dev_removed_ptxin_disconnect
...
void app_ptu_latching_fault_detected_handle(uint8_t latching_fault_type)
{
    app_ptu_state_set_with_notify(APP_PTU_LATCHING_FAULT_STATE);
}
...
void app_ptu_gtl_send_update_of_wdev_tab(void)
{
    ...
    for (i = 0; i < PTU_IMPL_MAX_SUPPORTED_DEV; ++i)
    {
        if (app_ptu_env.wpts_devices[i].used)
        {
            if (tab_msg_idx < wpts_dev_count)
            {
                msg->wpts_devices[tab_msg_idx] = app_ptu_env.wpts_devices[i].pub_data;
                tab_msg_idx++;
            }
            else
                break;// not enough free slots to get all devices
        }
    }
    msg->wpts_dev_count = wpts_dev_count;
...
{
    set_state_all_devices(DEV_PTU_IDLE);
    app_ptu_state_set_with_notify(APP_PTU_LATCHING_FAULT_STATE);
